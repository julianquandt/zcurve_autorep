...
if (file.exists(paste0(download_dir, journal_path, "/", "manual_download_list.bib"))) {
  bib_file <- readLines(paste0(download_dir, journal_path, "/", "manual_download_list.bib"))
  # replace @ with @article
  bib_file <- gsub("@{", "@article{", fixed = TRUE, bib_file)
  # write file
  writeLines(bib_file, paste0(download_dir, journal_path, "/", "manual_download_list.bib"))
} else {
...
for (name in names(pubdata_dfs)) {
  ...
  for (i in last_i:nrow(bib)) {
    ...
    if(is.na(bib$doi[i])){
      ...
      bib_file <- readLines(paste0(download_dir, journal_path, "/", "manual_download_list.bib")) # if multiple journals are used (e.g. when querying papers differently, journal_path might instead be a project path)
...
source_overview <- read.table(paste0(download_dir, journal_path, "/source_overview.csv"))
