...
```{r}
extract_tests <- function(path, files, select_results_only = FALSE, filter_phrases = c("hypothes", "prediction", "predicted", "anticipated", "surpris")) {
  ...
  for (file in files) {
    ...
    for (name in names(result_sections)) {
      ...
      if(!is.null(patterns)){
        ...
        selected_paragraphs[[name]] <- paragraphs
      }
    }
  ...
  df$p_value_num <- readr::parse_number(df$p_value)
...
```{r}
path <- here::here("downloads/Psychological_Science") # remember to replace / by \\ on windows
files_xml <- list.files(path, pattern = "*.xml", full.names = TRUE)

extracted_data <- extract_tests(path, files_xml, filter_phrases = c())
...
length(unique(extracted_data$file))-sum(extracted_data$p_value == "no p-values found; paper might be qualitative?")
...
# Save Extracted P-value Data to file

```{r}
write.csv(extracted_data, here::here("data/PS_2024_testdata.csv"))
