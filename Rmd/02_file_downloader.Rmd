...
```{r}
download_file <- function(row, oa_email_key, journal_path = "", use_proxy = FALSE, proxy_address = "socks5://localhost:9051", source_overview = TRUE) {
...
for (i in last_i:nrow(bib)) {
  ...
  download_file(bib[i, ], oa_email_key, journal_path = journal_path, use_proxy = FALSE, proxy_address = "socks5://localhost:9051")
  print(paste0("downloaded (or added to manual download list) paper ", i, " of ", nrow(bib), " with doi: ", bib$doi[i]))
  # Sys.sleep(5)
}
# to make the manual_download_list.bib file importable by Zotero, we need to replace all @ with @article
